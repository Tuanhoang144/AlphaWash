...
import type { CustomerDTO, OrderDetailDTO, OrderResponseDTO, ServiceDTO, VehicleDTO } from "@/types/OrderResponse"
...
export function useCreateInvoice() {
  ...
  const getDetail = (): OrderDetailDTO => (formData.orderDetails?.[0] as OrderDetailDTO) ?? buildEmptyDetail()
  ...
  const setServices = (nextServices: ServiceDTO[]) => {
    const prevDetail = getDetail()
    onOrderDetailChange({
      ...prevDetail,
